...
class Plotter
  ...
  def gen_image_line_hms_large loc
    ...
    while seek < count
      ...
      db_seek = db.where('MOD(id,30) = 0') if d_right.to_date - d_left.to_date > 90
      db_seek = db_seek.all
      seek += MY_LIMIT
    ...
    Gnuplot.open do |gp|
      Gnuplot::Plot.new( gp ) do |plot|
        ...
        if loc[:days] <= 3
          plot.format 'x "%H:%M:%S"'
          plot.label "287 '#{Time.parse(arr_x[0]).to_date}' at '#{arr_x[0]}', graph -0.1 center"
          plot.label "288 '#{Time.parse(arr_x[-1]).to_date}' at '#{arr_x[-1]}', graph -0.1 center"
          plot.bmargin '5'
        ...
        if ARGV.index("eq")
          eq_e.each do |eq|
            ...
            plot.arrow "from \"#{x}\",graph 1.0 to \"#{x}\",graph 0.0 lw 3 linecolor rgb 'black'"
        ...
        # unless moon_e == []
          # moon_e.each do |moon|
            ...
            # #plot.arrow "from \"#{x}\",graph 0.95 to \"#{x}\",graph 1 nohead"
            # plot.style 'fill empty'
            # plot.obj "#{brezent} circle arc #{moon_type[moon[:type].to_sym][0]} fc rgb 'black' noclip"
            # plot.obj "#{brezent} circle at '#{x_moon}', graph 1.02 size screen 0.007  front"
            # brezent += 1
        ...
        # unless orb_e == []
          # orb_e.each do |orb|
            ...
            # x = x.strftime("%Y.%m.%d.%H:%M:%S")
            # #plot.arrow "from \"#{x}\",graph 0.95 to \"#{x}\",graph 1 nohead"
            # plot.label "#{brezent} '#{orb[:type]}' at '#{x}', graph 1.04 font \"arialbd,22\" center"
            # brezent += 1
          # end
        # end
